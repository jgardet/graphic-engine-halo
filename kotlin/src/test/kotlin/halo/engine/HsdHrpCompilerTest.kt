...
class HsdHrpCompilerTest {
    ...
    fun compilesGenericSceneToHrp() {
        ...
        val payload = HsdHrpCompiler().compile(scene)
    ...
    fun rowPositionsChildrenOnce() {
        ...
        assertContentEquals(byteArrayOf(0, 28, 0, 20, -1, -1, -1), pixelCommands[1].second)
    }

    @Test
    fun rejectsUnknownAndOversizedScenes() {
        assertFailsWith<IllegalArgumentException> {
            HsdHrpCompiler().compile(json.parseToJsonElement("""{"scene":{"children":[{"type":"video"}]}}"""))
        }
        assertFailsWith<IllegalArgumentException> {
            HsdHrpCompiler().compile(json.parseToJsonElement("""{"scene":{"children":[{"type":"point","x":256,"y":0}]}}"""))
        }
    }
